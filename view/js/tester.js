...
(function() {
  var activeColor, button, down, elements, gs, inited, movePathTo, prepareElements, stickMax, translateRegEx, up,
  ...
  prepareElements = function(els) {
    var arr, name;
    for (name in els) {
      if (!__hasProp.call(els, name)) continue;
      arr = els[name];
  ...
  down = function(el, text) {
    el.oldFill = el.style.fill;
    el.style.fill = activeColor;
    return text.setAttribute('class', 'active');
  };

  up = function(el, text) {
    el.style.fill = el.oldFill;
    return text.setAttribute('class', '');
  };
  ...
  button = function(blockElement, name) {
    ...
    blockElement.on("up", function() {
      return up(elements[name][0], elements[name][1]);
    });
  };
  ...
  gs.addEventListener('on', function(e) {
    console.log("Gamepad on", e.detail);
  });

  gs.addEventListener('off', function(e) {
    console.log("Gamepad off", e.detail);
  });

  gs.on("on", function(e) {
    var LSX, LSY, RSX, RSY, pad;
    if (inited) {
      return;
    }
    pad = e.detail;
    ...
    LSX = 0;
    LSY = 0;
    pad.axes.LSX.on("change", function(e) {
      var x, y;
    ...
    pad.axes.LSY.on("change", function(e) {
      var x, y;
    ...
    RSX = 0;
    RSY = 0;
    pad.axes.RSX.on("change", function(e) {
      var x, y;
    ...
    pad.axes.RSY.on("change", function(e) {
      var x, y;
